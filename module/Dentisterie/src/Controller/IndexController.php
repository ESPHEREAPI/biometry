...
namespace Dentisterie\Controller;
...
class IndexController extends BackOfficeCommonController
{
    ...
    public function initialiserControlleur()
    {
        ...
        $this->peuxAjouter = $this->prestationManager->verifierSiPeuxAjouterDentisterie($this->tabListeMenu, $this->numeroOrdre);
    ...
    public function indexAction ()
    {
    	...
    	if( $this->naturePrestation == "dentisterie")
    	{
    	    $this->nomPage = $this->getTranslator("Liste des prestations en dentisterie");
    ...
    public function ajouterAction ()
    {
    	...
    	if( $this->naturePrestation == "dentisterie")
		{
			$this->nomPage = $this->getTranslator("Ajouter les prestations en dentisterie");
		}
		...
		if($this->naturePrestation == "dentisterie")
		 {
		 $listeTypePrestation=$this->typePrestationManager->getListeTypePrestation("1","dentisteries");	
	     $listeDescriptionSoins=$this->typePrestationManager->getListeTypePrestation("1","descriptionSoins");
    	...
    	return new ViewModel(array(
			...
			'listeDescriptionSoins'=>$listeDescriptionSoins
        ...
        if(!empty($postValues["listeLignePrestationSent"]))
        {
            ...
            foreach ($tabLignePrestation as $key => $valueIndex)
            {
				...
				$tabDsoins=array();
			        ...
			        $listeDs=$this->typePrestationManager->getListeTypePrestation("1","descriptionSoins");
					
					foreach($listeDs as $value) 
					{ 
					$tabDsoins[$value->getId()]= $value->getNom();
					}
					
								
					$form->add(array(
 		                    'name' =>  "descriptionSoinsTemplate_".$valueIndex,
 		                    'type' => 'Zend\Form\Element\Select',
 		                    'attributes' => array(
 			                'type' => 'select',
 			                'id' =>  "descriptionSoinsTemplate_".$valueIndex,
 			                "class" => "required",
			...
			$form->get("descriptionSoinsTemplate_".$valueIndex)->setValueOptions($tabDsoins);
                ...
                if($this->naturePrestation == "dentisterie")
                {
			        ...
			        $listeTp=$this->typePrestationManager->getListeTypePrestation("1","dentisteries");
			...
			$form->get("typeExamenTemplate_".$valueIndex)->setValueOptions($tabTpres);
				...
				$form->add(array(
                    "name" => "dentsConcerneesTemplate_".$valueIndex,
                    "attributes" => array(
                        "type" => "text",
                    ),
                ));
				...
				$form->add(array(
                    "name" => "codificationTemplate_".$valueIndex,
                    "attributes" => array(
                        "type" => "text",
                    ),
                ));
                ...
                $tabDs=array();
			        ...
			        $listDs=$this->typePrestationManager->getListeTypePrestation("1","descriptionSoins");
		          
					
					foreach($listeDs as $value) { 
					$tabDs[]= $value->getId();
					}
                    $prestationInputFilter->getInputFilter()->merge(new EnumImputFilterLcl("descriptionSoinsTemplate_".$valueIndex, true, $tabDs));
				...
				$prestationInputFilter->getInputFilter()->merge(new TextImputFilterLcl("dentsConcerneesTemplate_".$valueIndex, true, 1, 255));
				
				$prestationInputFilter->getInputFilter()->merge(new TextImputFilterLcl("codificationTemplate_".$valueIndex, true, 1, 255));
            }
        }
            ...
            if($form->isValid())
            {
                ...
                elseif($action == 'ajout' && $oldPrestation)
                {
                    if($this->naturePrestation == "dentisterie")
                    {
							...
							$error = $this->getTranslator("Vous avez deja enregistre des prestations en dentisterie pour cette visite");
                ...
                elseif($action == 'modif' && !$oldPrestation)
                {
                    if($this->naturePrestation == "dentisterie")
                    {
						...
						$error = $this->getTranslator("Impossible de trouver ces prestations en dentisterie");	
                       ...
                       if(!empty($postValues["listeLignePrestationSent"]))
                         {
                            ...
                            foreach ($tabLignePrestation as $key => $valueIndex)
                            {
                                if((( $this->naturePrestation == "dentisterie")  && !empty($donneesFormulaire["descriptionSoinsTemplate_".$valueIndex]) && !empty($donneesFormulaire["typeExamenTemplate_".$valueIndex]) &&
								!empty($donneesFormulaire["dentsConcerneesTemplate_".$valueIndex]) &&
							    !empty($donneesFormulaire["codificationTemplate_".$valueIndex])))
                                {
                                    ...
                                    $lignePrestation->setDescriptionSoins($this->getEntityManager()->find('Entity\TypePrestation', $donneesFormulaire["descriptionSoinsTemplate_".$valueIndex]));
                                   ...
                                   if( $this->naturePrestation == "dentisterie")
									...
									$lignePrestation->setDentsConcernees($donneesFormulaire["dentsConcerneesTemplate_".$valueIndex]);
									$lignePrestation->setCodification($donneesFormulaire["codificationTemplate_".$valueIndex]);
    ...
    public function modifierAction ()
    {
    	...
    	if($this->naturePrestation == "dentisterie")
    	{
			$this->nomPage = $this->getTranslator("Modifier les prestations en dentisterie");	
...
//     	}
    	...
    	foreach ($tabObjetLignePrestation as $unObjetLignePrestation)
    	{
	        ...
	        $tabLignePrestation[] = array("id" => $unObjetLignePrestation->getId(),
	            "descriptionSoins" => $unObjetLignePrestation->getdescriptionSoins()->getId(),
	            "prestation" => $unObjetLignePrestation->getPrestation()->getId(),
	            "nbre" => $unObjetLignePrestation->getNbre(),
	            "typeExamen" => $idTypeExamen,
				"dentsConcernees"=>$unObjetLignePrestation->getDentsConcernees(),
				"codification"=>$unObjetLignePrestation->getCodification(),						  
    ...
    public function paginationAction()
    {
    	   ...
    	   && isset($postValues['nbreMaxLigneTableau']))
    	{
    		...
    		if(is_array($tab) && count($tab) > 0 && $totalResult > 0)
    		{
    			...
    			include_once __DIR__.'/../../view/dentisterie/index/pagination.phtml';
            ...
            else
            {
                ...
                if(!$consultation)
                {
                    if($this->naturePrestation == "dentisterie")
                    {
						...
						$error = $this->getTranslator("Veuillez enregistrer la consultation pour cette visite avant d'enregistrer les prestations en dentisterie");	
                    }
                    else
                    {
                ...
                elseif($consultation->getEtatConsultation() != "valide" && $consultation->getEtatConsultation() != "encaisse")
                {
                    if( $this->naturePrestation == "dentisterie")
                    {
							...
							$error = $this->getTranslator("Cette consultation n'a pas encore ete validee, vous ne pouvez pas enregistrer des prestations en dentisterie pour celle-ci");
        ...
        if($form->isValid())
        {
            ...
            else
            {
                ...
                if(!$prestation)
                {
                    if($this->naturePrestation == "dentisterie")
                    {
							 $error = $this->getTranslator("Aucune prescription en dentisterie n'a ete enregistre pour cette visite");
    ...
    public function imprimerRecuAction()
    {
        ...
        if(empty($error))
        {
            ...
            if(empty($error))
            {
                ...
                include_once __DIR__.'/../../view/dentisterie/index/imprimer-recu.phtml';
            }
        }
        else
        {
...
//             );
            ...
            {
                include_once __DIR__.'/../../view/dentisterie/index/imprimer-pdf.phtml';
            }
            else
            {
                $info = $this->getTranslator("Aucun element trouve");
    ...
    public function imprimerDetaillePdfAction()
    {
            ...
            && isset($postValues['nomColoneTriPagination']) && isset($postValues['typeTriColonePagination']))
        {
            ...
            if(is_array($tab) && count($tab) > 0)
            {
                include_once __DIR__.'/../../view/dentisterie/index/imprimer-detaille-pdf.phtml';
            }
            else
            {
                $info = $this->getTranslator("Aucun element trouve");
