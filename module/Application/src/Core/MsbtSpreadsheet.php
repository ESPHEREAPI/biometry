...
class MsbtSpreadsheet
{
	...
	public function __construct(array $properties, array $titresColones, array $datas, $nomFichier, $numPremiereLigne=1, $numPremiereColone=0)
	{
		...
		$this->setObjPHPExcel(new Spreadsheet());
		...
		$this->objPHPExcel->getActiveSheet()->getParent()->getDefaultStyle()->applyFromArray($style);
	...
	protected function setTitresColones($titresColones)
	{
		...
		foreach($this->titresColones as $uneColone)
		{
			...
			{
				$stringColone = Coordinate::stringFromColumnIndex($indiceColone);
...
// 					            )
			...
			$stringPremiereColone = Coordinate::stringFromColumnIndex($this->numPremiereColone);
			$stringDerniereColone = Coordinate::stringFromColumnIndex($this->numPremiereColone + count($this->titresColones) -1);
	...
	public function constructLine($datasLigne, $indiceLigne, $indiceColone=0)
	{
		...
		if(isset($datasLigne["titre"]) && isset($datasLigne['donnees']))
		{
			...
			if(is_array($datasLigne['donnees']) && count($datasLigne['donnees']) > 0)
			{
				...
				// Merge columns
				$stringColone = Coordinate::stringFromColumnIndex($indiceColoneInit);
...
// 		$objWriter = IOFactory::createWriter($this->getObjPHPExcel(), 'Excel2007');
...
// 		\PHPExcel_Settings::setCacheStorageMethod($cacheMethod, $cacheSettings);
		...
		$objWriter = IOFactory::createWriter($this->getObjPHPExcel(), 'Xlsx');
